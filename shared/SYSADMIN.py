...
# Ruta de importación actualizada según la nueva estructura de carpetas
from shared.Tools.openrouter.mcp.balance_manager import check_openrouter_balance
...
def read_file(path):
    ...
    try:
        with open(path.strip(), "r", encoding="utf-8") as f:
            contenido = f.read()
            # Limitamos la lectura a los últimos 3500 caracteres para proteger la memoria 
            # de GarAI en caso de abrir logs masivos.
            return contenido[-3500:] if len(contenido) > 3500 else contenido
    except Exception as e:
...
def run_command(command):
    """Ejecuta un comando de terminal (Linux, PM2, Cron, Tree, etc)."""
    try:
        # Límite aumentado a 3500 para leer outputs largos sin romper el formato en Telegram
        ...
        return f"Error en comando (Código {e.returncode}): {e.output[:3500]}"
    except Exception as e:
        return f"Error al ejecutar: {str(e)}"

# Diccionario unificado para mapear nombres de herramientas a funciones reales
AVAILABLE_TOOLS = {
    "list_files": list_files,
    "read_file": read_file,
    "write_file": write_file,
    "run_command": run_command,
    "check_openrouter_balance": check_openrouter_balance
}